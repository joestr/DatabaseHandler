...
public class SQLiteEntryHandler extends EntryHandler {
    ...
    protected void addTable(TableEntry tableEntry) throws SQLException {
        ...
        while (iterator.hasNext()) {
            ...
            columnBuilder.append(column.autoIncrement() || column.columnType() == ColumnType.Primary ? "INTEGER"
                    : this.toDatabaseType(
                            column.entryType() != EntryType.Normal ? DataType.String : column.dataType()));
            if (column.columnType() != ColumnType.Normal)
                columnBuilder
                        .append(column.columnType() == ColumnType.Primary ? " PRIMARY KEY AUTOINCREMENT" : " UNIQUE");
            if (iterator.hasNext())
                columnBuilder.append(", ");
        }

        preparedStatement = connection.prepareStatement(
                "CREATE TABLE IF NOT EXISTS `" + tableEntry.getTableName() + "` (" + columnBuilder.toString() + ");");
