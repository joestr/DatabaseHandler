...
public class MySQLEntryHandler extends EntryHandler {
    ...
    private void insert(TableEntry tableEntry, Connection connection)
        ...
        if (hasPrimaryKey) { // TODO - handle this when having multible auto increments
            ...
            if (resultSet.next())
                tableEntry.setColumn(primaryKey, resultSet.getInt(1));
        }
    ...
    public boolean update(TableEntry tableEntry, Map<String, Object> filterList, List<String> specificRows)
        ...
        while (setIterator.hasNext()) {
            ...
            if (column.columnType() == ColumnType.Primary || column.autoIncrement())
                continue;
    ...
    protected void addTable(TableEntry tableEntry) throws SQLException {
        ...
        while (iterator.hasNext()) {
            ...
            columnBuilder.append(
                    this.toDatabaseType(column.entryType() != EntryType.Normal ? DataType.String : column.dataType()));
            if (column.autoIncrement())
                columnBuilder.append(" AUTO_INCREMENT");
            if (column.columnType() != ColumnType.Normal)
                columnBuilder
                        .append(column.columnType() == ColumnType.Primary ? " PRIMARY KEY AUTO_INCREMENT" : " UNIQUE");

            if (iterator.hasNext())
                columnBuilder.append(", ");
        }

        preparedStatement = connection.prepareStatement("CREATE TABLE IF NOT EXISTS `" + tableEntry.getTableName()
                + "` (" + columnBuilder.toString() + ") CHARACTER SET utf8 COLLATE utf8_general_ci;");
    ...
    protected void addColumn(TableEntry tableEntry, Column column) throws SQLException {
        ...
        String whereToAdd = column.order() == -1 ? ""
                : column.order() == 1 && layout.size() > 1
                        ? " BEFORE `" + tableEntry.getColumn(column.order() + 1).columnName() + "`"
                        : column.order() > 1 && layout.size() >= column.order()
                                ? " AFTER `" + tableEntry.getColumn(column.order() - 1).columnName() + "`"
                                : "";
        ...
        PreparedStatement preparedStatement = connection.prepareStatement("ALTER TABLE `" + tableEntry.getTableName()
                ...
                + (column.columnType() == ColumnType.Unique ? " UNIQUE" : "") + whereToAdd + ";");
        preparedStatement.execute();
