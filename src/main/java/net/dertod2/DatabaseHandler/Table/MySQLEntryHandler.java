...
public class MySQLEntryHandler extends EntryHandler {
	...
	protected void addTable(TableEntry tableEntry) throws SQLException { 
		...
		while (iterator.hasNext()) {
			...
			columnBuilder.append(this.toDatabaseType(column.entryType() != EntryType.Normal ? DataType.String : column.dataType()));
			if (column.autoIncrement()) columnBuilder.append(" AUTO_INCREMENT");
			if (column.columnType() != ColumnType.Normal) columnBuilder.append(column.columnType() == ColumnType.Primary ? " PRIMARY KEY AUTO_INCREMENT" : " UNIQUE");
			
			if (iterator.hasNext()) columnBuilder.append(", ");
		}
		
		preparedStatement = connection.prepareStatement("CREATE TABLE IF NOT EXISTS `" + tableEntry.getTableName() + "` (" + columnBuilder.toString() + ") CHARACTER SET utf8 COLLATE utf8_general_ci;");
