...
public abstract class TableEntry {
	...
	public String getTableName() {
		...
		if (tableInfo != null) return tableInfo.tableName();
	...
	/**
	 * This method will be called when succesfully and completely loaded this entry out of the database.<br />
	...
	public boolean hasPrimaryKey() {
		Field[] fieldList = TableEntry.safeArrayMerge(getClass().getFields(), getClass().getDeclaredFields(), new Field[1]);
		
		for (final Field field : fieldList) {
			if (!field.isAnnotationPresent(Column.class)) continue;
			Column columnInfo = field.getAnnotation(Column.class);
			if (columnInfo.columnType() == ColumnType.Primary) return true;
		}
	...
	public Column getPrimaryKey() {
		...
		for (final Field field : fieldList) {
			...
			if (column.columnType() == ColumnType.Primary) return column;
		}
	...
	public boolean hasUniqueKeys() {
		...
		for (final Field field : fieldList) {
			if (!field.isAnnotationPresent(Column.class)) continue;
			Column column = field.getAnnotation(Column.class);
			if (column.columnType() == ColumnType.Unique) return true;
		}
    ...
    public static class ColumnFieldSorter implements Comparator<Field> {
		...
		public int compare(Field o1, Field o2) {
			if (!o1.isAnnotationPresent(Column.class)) return 0;
			if (!o2.isAnnotationPresent(Column.class)) return 0;
