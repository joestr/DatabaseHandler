...
public @interface Column {
    public enum DataType {
        ...
        /**
         * Binary Data as of java Type <b>byte[]</b>. Saved in database mysql as
         * <b>BLOB</b> and postgres as <b>bytea</b>
         */
        Binary(null, null);
        ...
        public static DataType byClass(Class<?> type) {
            for (DataType dataType : DataType.values()) {
                if (type.isAssignableFrom(dataType.cls)
                        || (dataType.primCls != null && type.isAssignableFrom(dataType.primCls))) {
                    return dataType;
                }
            }
    ...
    /**
     * Sets this column to an Primary Key or Unique Key< br/> When left empty this
     * will be an normal column
     */
    ColumnType columnType() default ColumnType.Normal;

    /**
     * Should the database automatically increment this variable when an new element
    ...
    /**
     * Allows to order the columns so the database driver creates an fancy table
     * without random order<br />
