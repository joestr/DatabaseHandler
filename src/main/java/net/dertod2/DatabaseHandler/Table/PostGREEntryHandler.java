...
public class PostGREEntryHandler extends EntryHandler {
	...
	private void insert(TableEntry tableEntry, Connection connection) throws IllegalArgumentException, IllegalAccessException, SQLException {
		if (this.copyInsert(ImmutableList.<TableEntry>of(tableEntry), connection)) return;
	...
	private boolean copyInsert(List<TableEntry> entryList, Connection connection) {
		try {
			...
			// The copyRows method only works with data WITHOUT an primary Key or "return variables"
			
			Map<String, List<TableEntry>> tableEntryList = new HashMap<String, List<TableEntry>>(); 
			for (TableEntry tableEntry : entryList) {
				if (tableEntry.hasPrimaryKey()) return false;
				
				if (!tableEntryList.containsKey(tableEntry.getTableName())) tableEntryList.put(tableEntry.getTableName(), new ArrayList<TableEntry>());
		...
		while (columnIterator.hasNext()) {
			...
			if (column.entryType() != EntryType.Normal) {
				if (column.entryType() == EntryType.Map) {
					tableEntry.setColumn(column, this.getMap(column.dataType(), column.entryDataType(), resultSet.getString(column.columnName())));
				} else {
					tableEntry.setColumn(column, this.getList(column.dataType(), resultSet.getString(column.columnName())));
				}
			} else {
	...
	protected void addTable(TableEntry tableEntry) throws SQLException {
		...
		while (iterator.hasNext()) {
			...
			columnBuilder.append(column.autoIncrement() || column.columnType() == ColumnType.Primary ? "BIGSERIAL" : this.toDatabaseType(column.entryType() != EntryType.Normal ? DataType.String : column.dataType()));
			if (column.columnType() != ColumnType.Normal) columnBuilder.append(column.columnType() == ColumnType.Primary ? " PRIMARY KEY" : " UNIQUE");
			
			if (iterator.hasNext()) columnBuilder.append(", ");
		}

		preparedStatement = connection.prepareStatement("CREATE TABLE " + tableEntry.getTableName() + " (" + columnBuilder.toString() + ");");
	...
	protected void addColumn(TableEntry tableEntry, Column column) throws SQLException {
		...
		//String before = column.order() != -1 ? " BEFORE " + tableEntry.getColumn(column.order() - 1).columnName() : "";
		PreparedStatement preparedStatement = connection.prepareStatement("ALTER TABLE " + tableEntry.getTableName() + " ADD " + column.columnName() + " " + this.toDatabaseType(dataType) + (column.columnType() == ColumnType.Unique ? " UNIQUE" : "") + ";");
		preparedStatement.execute();
