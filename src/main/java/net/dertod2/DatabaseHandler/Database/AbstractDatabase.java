    ...
    public AbstractDatabase(String host, Integer port, String database, String username, String password)
        ...
        if (this.getType().isUsingDatabaseDriver()) {
            try {
                ...
                Bukkit.getConsoleSender().sendMessage(ChatColor.GREEN + "Database Driver '" + ChatColor.GOLD
                        + this.getType().name() + ChatColor.GREEN + "' successfully loaded...");
            ...
            if (connection != null) {
                Bukkit.getConsoleSender()
                        .sendMessage(ChatColor.GREEN + "Successfully etablished test connection to database. :-)");
                try {
                    connection.close();
            ...
            Bukkit.getConsoleSender()
                    .sendMessage(ChatColor.GREEN + "Database Type '" + ChatColor.GOLD + this.getType().name()
                            + ChatColor.GREEN + "' doesn't support Connections. Skip initializing BoneCP.");
        }
    }
