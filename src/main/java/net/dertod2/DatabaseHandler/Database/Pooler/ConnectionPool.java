...
public class ConnectionPool {
	...
	private volatile List<PooledConnection> availableList = new CopyOnWriteArrayList<PooledConnection>(); //new ArrayList<PooledConnection>();
	private volatile List<PooledConnection> loanedList = new CopyOnWriteArrayList<PooledConnection>(); // new ArrayList<PooledConnection>();
	...
	public void startPool() {
		if (!this.testCredentials()) return;
		
		this.watchThread = new Thread(new Runnable() { public void run() { thread(); } });
		this.watchThread.setPriority(Thread.MAX_PRIORITY);
		this.watchThread.setDaemon(true);
		
		this.watchThread.start();
		try { Thread.sleep(500L); } catch (Exception exc) { } // Let the main thread sleep for 500 ms
	}
	...
	private void thread() {
		...
		while (this.watchThread == currentThread) {
			...
			try {
				...
				for (PooledConnection pooledConnection : this.loanedList) {
					try {
						if (pooledConnection.returnToPool) { // Returns connection to the Pool
							...
							if (this.debugMode) System.out.println("Returned Pooled Connection to the Pool (fetcher executed close method)...");
						} else if (pooledConnection.getLoanedtime() > this.poolSettings.maxLoanedTime && pooledConnection.autoClose) {
							try { pooledConnection.rawConnection.close(); } catch (SQLException exc) { }
							removeableList.add(pooledConnection);
							this.poolStatistics.maxLoanedTimeReached++;
							if (this.debugMode) System.out.println("Force closed Pooled Connection 'cause of maxLoanedtime reached...");
						}
				...
				for (PooledConnection pooledConnection : this.availableList) {
					try {
						if (pooledConnection.getLifetime() >= this.poolSettings.maxLifetime) {
							...
							if (this.debugMode) System.out.println("Closed Pooled Connection 'cause of maxLifetime reached...");
						} else if (pooledConnection.getIdletime() >= this.poolSettings.maxIdletime) {
							removeableList.add(pooledConnection);
							this.poolStatistics.maxIdleTimeReached++;
							if (this.debugMode) System.out.println("Closed Pooled Connection 'cause of maxIdletime reached...");
						} else if (!pooledConnection.rawConnection.isValid(1)) {
							removeableList.add(pooledConnection);
							this.poolStatistics.invalidConnection++;
							if (this.debugMode) System.out.println("Closed Pooled Connection 'cause of invalid raw connection...");
						}
					} catch (Exception exc) { }
				}
				...
				if (this.getLastFetchTime() < this.poolSettings.startSleepMode) {
					while (this.availableList.size() < this.poolSettings.minPoolSize) {
						...
						if (connection != null) {
							...
							if (this.debugMode) System.out.println("Opened new Pooled Connection cause not enough available Connections");
						}	
					}
				}
				...
				try { Thread.sleep(this.poolSettings.watcherTimer); } catch (Exception exc) { }
			} catch (Exception exc) {
				exc.printStackTrace();
			}
		}
	 ...
	 * Warning: When no connection is available but the Pool can open more connections this will end in an Thread lock<br />
	...
	public Connection getConnection() {
		...
		if (this.availableList.isEmpty()) {
			...
			if (openedConnections >= this.poolSettings.maxPoolSize) {
				...
				while (this.forceWatch) { } // Thread Locking
			}
		}
		...
		if (this.debugMode) System.out.println("Fetched Pooled Connection out of pool...");
	...
	private String getFetcher(StackTraceElement[] stackTrace) {
		...
		for (StackTraceElement stackTraceElement : stackTrace) {
			...
			if (className.contains("net.dertod2") && !className.contains("DatabaseHandler")) {
				int beginIndex = className.indexOf(".", className.indexOf(".")) + 1; // plugin names -> ex.: net.dertod2.DatabaseHandler or me.author.pluginname
