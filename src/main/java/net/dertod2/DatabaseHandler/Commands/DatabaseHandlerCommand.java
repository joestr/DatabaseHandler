...
public class DatabaseHandlerCommand implements TabExecutor {
    private static long second = 1000, minute = 60000, hour = 3600000, day = 86400000,
            month = NumberUtils.toLong("2592000000"), year = NumberUtils.toLong("31104000000");
    ...
    private final Map<String, String> tabCategoryMap = ImmutableMap.<String, String>builder()
            ...
            .put("list", "databasehandler.commands.databasehandler.list").build();
    ...
    public DatabaseHandlerCommand() {
        ...
        this.helpMap.put("databasehandler.commands.databasehandler.reload",
                new ComponentBuilder("/dh ").color(ChatColor.DARK_GRAY).append("reload").color(ChatColor.GRAY)
                        .event(new HoverEvent(HoverEvent.Action.SHOW_TEXT,
                                TextComponent.fromLegacyText("Reloads the plugin and settings")))
                        .event(new ClickEvent(ClickEvent.Action.SUGGEST_COMMAND, "/databasehandler:dh reload"))
                        .create());
        this.helpMap.put("databasehandler.commands.databasehandler.restart",
                new ComponentBuilder("/dh ").color(ChatColor.DARK_GRAY).append("restart").color(ChatColor.GRAY)
                        .event(new HoverEvent(HoverEvent.Action.SHOW_TEXT,
                                TextComponent.fromLegacyText("Restarts the database pool")))
                        .event(new ClickEvent(ClickEvent.Action.SUGGEST_COMMAND, "/databasehandler:dh restart"))
                        .create());
        this.helpMap.put("databasehandler.commands.databasehandler.update",
                new ComponentBuilder("/dh ").color(ChatColor.DARK_GRAY).append("update").color(ChatColor.GRAY)
                        .event(new HoverEvent(HoverEvent.Action.SHOW_TEXT,
                                TextComponent.fromLegacyText("Searches for new versions of this plugin")))
                        .event(new ClickEvent(ClickEvent.Action.SUGGEST_COMMAND, "/databasehandler:dh update"))
                        .create());
        this.helpMap.put("databasehandler.commands.databasehandler.stats",
                new ComponentBuilder("/dh ").color(ChatColor.DARK_GRAY).append("stats").color(ChatColor.GRAY)
                        .event(new HoverEvent(HoverEvent.Action.SHOW_TEXT,
                                TextComponent.fromLegacyText("Shows different statistics over the connection pool")))
                        .event(new ClickEvent(ClickEvent.Action.SUGGEST_COMMAND, "/databasehandler:dh stats"))
                        .create());
        this.helpMap.put("databasehandler.commands.databasehandler.list",
                new ComponentBuilder("/dh ").color(ChatColor.DARK_GRAY).append("list").color(ChatColor.GRAY)
                        .event(new HoverEvent(HoverEvent.Action.SHOW_TEXT,
                                TextComponent.fromLegacyText("Shows informations over loaned connections")))
    ...
    public boolean onCommand(CommandSender sender, Command command, String alias, String[] args) {
        if (args.length == 1 && args[0].equalsIgnoreCase("reload")
            ...
            sender.sendMessage(ChatColor.GREEN + "Plugin and configuration reloaded!");
                ...
                && sender.hasPermission(this.tabCategoryMap.get("restart"))) {
            ...
            sender.sendMessage(ChatColor.GREEN + "Checking for updates... When no further" +
            "messages appears there are no updates available!");
            DatabaseHandler.updater.check(sender);
        } else if (args.length == 1 && args[0].equalsIgnoreCase("stats")
                && sender.hasPermission(this.tabCategoryMap.get("stats"))) {
            if (sender instanceof Player) {
                ...
                && sender.hasPermission(this.tabCategoryMap.get("list"))) {
            if (sender instanceof Player) {
    ...
    private void stats(Player sender) {
        ...
        BaseComponent[] general = new ComponentBuilder("Settings: ").color(ChatColor.DARK_GREEN)
                ...
                .event(new HoverEvent(HoverEvent.Action.SHOW_TEXT,
                        new ComponentBuilder("").append("Smallest Pool Size: ").color(ChatColor.DARK_GREEN)
                                .append(String.valueOf(poolSettings.getMinimumPoolSize())).color(ChatColor.GOLD)
                                .append("\n").append("Biggest Pool Size: ").color(ChatColor.DARK_GREEN)
                                .append(String.valueOf(poolSettings.getMaximumPoolSize())).color(ChatColor.GOLD)
                                .append("\n").append("Minimum Available: ").color(ChatColor.DARK_GREEN)
                                .append(String.valueOf(poolSettings.getMinimumAvailable())).color(ChatColor.GOLD)
                                .append("\n").append("Highest Idle-Time: ").color(ChatColor.DARK_GREEN)
                                .append(String.valueOf(poolSettings.getIdletime())).color(ChatColor.GOLD).append("\n")
                                .append("Maximum Life-Time: ").color(ChatColor.DARK_GREEN)
                                .append(String.valueOf(poolSettings.getLifetime())).color(ChatColor.GOLD).append("\n")
                                .append("Time until Sleep-Mode: ").color(ChatColor.DARK_GREEN)
                                .append(String.valueOf(poolSettings.getStartSleepMode())).color(ChatColor.GOLD)
                                .create()))
                .create();
        ...
        sender.sendMessage("");
        sender.sendMessage(
                ChatColor.DARK_GREEN + "Count of watcher loops: " + ChatColor.GOLD + poolStatistics.getWatcherRuns());
        sender.sendMessage(ChatColor.DARK_GREEN + "Watcher loop duration: " + ChatColor.GOLD
                + poolStatistics.getLastWatcherDuration() + "ms");
        sender.sendMessage(
                ChatColor.DARK_GREEN + "Returned to pool: " + ChatColor.GOLD + poolStatistics.getReturnedToPool());
        sender.sendMessage(ChatColor.DARK_GREEN + "Time since last request: " + ChatColor.GOLD
                + timeToString(connectionPool.getLastFetchTime()));
        sender.sendMessage(ChatColor.DARK_GREEN + "Thread Locks: " + ChatColor.GOLD + poolStatistics.getThreadLock());
        sender.sendMessage(ChatColor.DARK_GREEN + "Sleep-Mode: " + ChatColor.GOLD
                + (connectionPool.getLastFetchTime() >= poolSettings.getStartSleepMode() ? "Active" : "Inactive"));
        sender.sendMessage("");

        BaseComponent[] connections = new ComponentBuilder("Connections: ").color(ChatColor.DARK_GREEN)
                .append("-> Show here <-").color(ChatColor.GOLD)
                .event(new HoverEvent(HoverEvent.Action.SHOW_TEXT, new ComponentBuilder("").append("Created: ")
                        .color(ChatColor.DARK_GREEN).append(String.valueOf(poolStatistics.getOpenedConnections()))
                        .color(ChatColor.GOLD).append("\n").append("----------").append("Active: ")
                        .color(ChatColor.DARK_GREEN)
                        .append(String.valueOf(
                                connectionPool.getAvailableConnections() + connectionPool.getLoanedConnections()))
                        .color(ChatColor.GOLD).append("\n").append("Loaned: ").color(ChatColor.DARK_GREEN)
                        .append(String.valueOf(connectionPool.getLoanedConnections())).color(ChatColor.GOLD)
                        .append("\n").append("In pool: ").color(ChatColor.DARK_GREEN)
                        .append(String.valueOf(connectionPool.getAvailableConnections())).color(ChatColor.GOLD)
                        .create()))
                .create();
        sender.spigot().sendMessage(connections);

        int closedConnections = poolStatistics.getMaxIdleTimeReached() + poolStatistics.getMaxLifeTimeReached()
                + poolStatistics.getMaxLoanedTimeReached() + poolStatistics.getInvalidConnection();
        BaseComponent[] closed = new ComponentBuilder("Closed Connections: ").color(ChatColor.DARK_GREEN)
                .append(String.valueOf(closedConnections) + " ").color(ChatColor.GOLD).append("-> Show here <-")
                .color(ChatColor.GOLD)
                .event(new HoverEvent(HoverEvent.Action.SHOW_TEXT,
                        new ComponentBuilder("").append("By reaching idle-time: ").color(ChatColor.DARK_GREEN)
                                .append(String.valueOf(poolStatistics.getMaxIdleTimeReached())).color(ChatColor.GOLD)
                                .append("\n").append("By reaching life-time: ").color(ChatColor.DARK_GREEN)
                                .append(String.valueOf(poolStatistics.getMaxLifeTimeReached())).color(ChatColor.GOLD)
                                .append("\n").append("By reaching loaned-time: ").color(ChatColor.DARK_GREEN)
                                .append(String.valueOf(poolStatistics.getMaxLoanedTimeReached())).color(ChatColor.GOLD)
                                .append("\n").append("By becoming invalid: ").color(ChatColor.DARK_GREEN)
                                .append(String.valueOf(poolStatistics.getInvalidConnection())).color(ChatColor.GOLD)
                                .create()))
                .create();
    ...
    private void list(Player sender) {
        ...
        if (connectionList.size() > 0) {
            ...
            // Vergebene Verbindung: <Aktive Zeit> {Hover: Idlezeit: Fetcher, Verliehene
            // Zeit}
            sender.sendMessage(ChatColor.DARK_GREEN + "There are " + ChatColor.GOLD + connectionList.size()
                    + ChatColor.DARK_GREEN + " Connections opened: ");
            for (PooledConnection pooledConnection : connectionList) {
                if (pooledConnection.isInPool()) {
                    sender.spigot()
                            .sendMessage(new ComponentBuilder("Available Connection: ").color(ChatColor.DARK_GREEN)
                                    .append(timeToString(pooledConnection.getLifetime())).color(ChatColor.GOLD)
                                    .event(new HoverEvent(HoverEvent.Action.SHOW_TEXT,
                                            new ComponentBuilder("Time since connection created").create()))
                                    .append(" (Show more)")
                                    .event(new HoverEvent(HoverEvent.Action.SHOW_TEXT,
                                            new ComponentBuilder("Idle: ").color(ChatColor.DARK_GREEN)
                                                    .append(timeToString(pooledConnection.getIdletime()))
                                                    .color(ChatColor.GOLD).create()))
                                    .create());
                } else {
                    sender.spigot().sendMessage(new ComponentBuilder("Loaned Connection: ").color(ChatColor.DARK_GREEN)
                            .append(timeToString(pooledConnection.getLifetime())).color(ChatColor.GOLD)
                            .event(new HoverEvent(HoverEvent.Action.SHOW_TEXT,
                                    new ComponentBuilder("Time since connection created").create()))
                            .append(" (Show more)")
                            .event(new HoverEvent(HoverEvent.Action.SHOW_TEXT,
                                    new ComponentBuilder("").append("Idle: ").color(ChatColor.DARK_GREEN)
                                            .append(timeToString(pooledConnection.getIdletime())).color(ChatColor.GOLD)
                                            .append("\n").append("Loaned to: ").color(ChatColor.DARK_GREEN)
                                            .append(pooledConnection.getFetcher()).color(ChatColor.GOLD).append("\n")
                                            .append("Loaned before: ").color(ChatColor.DARK_GREEN)
                                            .append(timeToString(pooledConnection.getLoanedtime()))
                                            .color(ChatColor.GOLD).append("\n").create()))
                            .create());
                }
            }
