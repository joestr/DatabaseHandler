 ...
 * + when inserting rows with an primary key the key in the entry will be updated automatically<br />
...
public class DatabaseHandler extends JavaPlugin {
	...
	public void onEnable() {
		...
		if (databaseType != null) {
			...
			try {
				...
				if (DatabaseHandler.abstractDatabase != null) DatabaseHandler.entryHandler = DatabaseHandler.abstractDatabase.getHandler();
			} catch (SQLException exc) {
				Bukkit.getConsoleSender().sendMessage(ChatColor.RED + "Fatal error while setting up the database Connection Pool");
				exc.printStackTrace();
			}
		} else {
			Bukkit.getConsoleSender().sendMessage(ChatColor.RED + "No database driver set! Please configure the config.yml");
			System.exit(-1);
		}
	...
	public static Connection getConnection() throws SQLException {
		if (DatabaseHandler.abstractDatabase == null) return null;
	 ...
	 * Only use the entry handler over the internal funtion inside the SQLiteDatabase Object!
	 ...
	 * @return EntryHandler
	 * @deprecated use the EntryHandler over the {@link AbstractDatabase#getHandler()} method
	 */
